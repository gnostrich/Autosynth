# nidhogg — a second instantiation of the v1 architecture

`nidhogg` is a **trained model**: the v1 ETS architecture (rev-r1 F, unit-resolved
...
v1 implementation lives here under `instantiations/nidhogg/`, so the v1 tree that
produced the first instantiation ("jörmungandr") is **never edited**.
...
| world | frozen, **M=5** anchors, bound to the nidhogg LAMBDA |
