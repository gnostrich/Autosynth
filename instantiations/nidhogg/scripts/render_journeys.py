#!/usr/bin/env python3
"""Render a set of knob-journey "tracks" from the nidhogg instantiation.
...
def main():
    ...
    print(f"[nidhogg] load world {world_path}", flush=True)
    wf = load_world(world_path)
    sigma = resolve_sigma(wf, None)     # fork's ets/calibration/sigma_phi.json
    print(f"[nidhogg] world M={wf.world.M} hash={wf.world_hash[:12]} "
          f"sigma_calibrated={sigma is not None}", flush=True)

    t0 = time.time()
    print("[nidhogg] materialize source bank (cached after first build) ...", flush=True)
    bank = build_bank(wf)
    print(f"[nidhogg] bank ready: {len(bank)} units in {time.time()-t0:.0f}s", flush=True)
    ...
    print("[nidhogg] all journeys done", flush=True)
